...
public:
  ...
  bool operator() (const Edge& lhs, const Edge& rhs) const
  {
	if (lhs.size == rhs.size) {
		if(reverse) return lhs.neighbour > rhs.neighbour;
...
bool edgeGreater(const Edge& e1, const Edge& e2) {
	...
	else if(e1.size == e2.size) return e1.neighbour > e2.neighbour;
	return false;
}
...
int addToSuitorsQueue(int u, int edgeSize, int p) {
	...
	if(suitors[p].size() >= bValue[p]) {
		...
		//cout<<originalNodesIds[u]<<" become suitor of "<<originalNodesIds[p]<<" edgeSize "<<edgeSize<<" annuled "<<originalNodesIds[annuledVertex]<<endl;
		suitors[p].pop();
	}
	else {
...
int makeUsuitorOfP(int u, int edgeSize, int p) {
	//if(lockSuitorsQueue) suitorsProtection[p].lock();	
	int annuledVertex = addToSuitorsQueue(u, edgeSize, p);
	//if(lockSuitorsQueue) suitorsProtection[p].unlock();
	return annuledVertex;
}
...
void setBValue(int generator, vector<int>& originalNodesIds) {
	...
	for(int i = 0; i < bValue.size(); i++) {
		//cout<<"vertex i b(i): "<<bValue[i]<<endl;
	}	
}
...
int sumOfvertexSuitors(int vertex) {
	int sum = 0;
	/*	
	for (const auto& s: suitors[vertex])
   		sum += s.size();
	*/
	...
	//cout<<"ilosc adoratorow suma : "<<suitors[vertex].size()<<"  "<<sum<<endl;
	return sum;
}
...
void getValuesFromLine(int& v1, int& v2, int& size, string& line) {
	...
	size = atoi(tmp.c_str());
	//cout<<"variables obtained v1 v2 size: "<<v1<<" "<<v2<<" "<<size<<endl;
}

//TO DO! dokoncz readfile, alokacje i inicacje wierzcholkow
...
void allocateAdjacentLists(vector<int>& vertexAdjacentEdges) {
	...
	for(int i = 0; i < vertexAdjacentEdges.size(); i++) {
		adjList[i].resize(vertexAdjacentEdges[i], Edge {0,0});
	}
}
...
void drukujAdjacentList() {
	...
	for(int i = 0; i < adjList.size(); i++) {
		...
		for(int j = 0; j < adjList[i].size(); j++) {
			cout<<"{"<<adjList[i][j].neighbour<<" , "<<adjList[i][j].size<<"}"<<"  ";
		}
		cout<<endl;
	}
}
